...
int main(int argc, char **argv) {
  ...
  // We also need to initialize g_num_prod_lock
...
void *producer_routine(void *arg) {
  ...
  // We need join the thread to get and free the return value stored on the heap
  ...
  --g_num_prod; // g_num_prod_lock should be acquired in order to modify this global variable shared by different threads
  pthread_mutex_unlock(&g_num_prod_lock);

  // free the return value from the consumer thread stored on heap
  // also, prevent unlock locks that has been destoryed by the main thread
  void *thread_return = NULL;
  result = pthread_join(consumer_thread, &thread_return);
  if (0 != result) {
    fprintf(stderr, "Failed to join consumer thread: %s\n", strerror(result));
...
void *consumer_routine(void *arg) {
  ...
  // we cannot return an address pointing to the stack
  ...
  *count = 0;
  ...
  while(queue_p->front != NULL || g_num_prod > 0) {
    ...
    // before checking the while condition, we need to acquire locks again.
    pthread_mutex_lock(&queue_p->lock);
    pthread_mutex_lock(&g_num_prod_lock);
  }
  pthread_mutex_unlock(&g_num_prod_lock);
